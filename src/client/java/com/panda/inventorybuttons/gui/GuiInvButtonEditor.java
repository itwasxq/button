...
public class GuiInvButtonEditor extends Screen {
        ...
        this.commandTextField.setResponder(s -> {
            if (editingButton != null) {
                ...
                if (text.isEmpty()) {
                    ...
                    commandTextField.setCursorPosition(1);
                } else if (!text.startsWith("/")) {
                    text = "/" + text.replace("/", "");
                    commandTextField.setValue(text);
                    commandTextField.setCursorPosition(text.length());
                }
    ...
    private void renderSkullInfoPanel(DrawContext context, int mouseX, int mouseY) {
        ...
        context.drawString(font, Component.literal("How to find Skull IDs").withStyle(ChatFormatting.YELLOW, ChatFormatting.BOLD), textX, textY, 0xFFFFFFFF);
        textY += 18;
    ...
    private void search(String query) {
        ...
        if (currentMode == FilterMode.ALL || currentMode == FilterMode.ITEMS || currentMode == FilterMode.BLOCKS) {
            for (Item item : BuiltInRegistries.ITEM) {
                ...
                if (id.toString().contains(lower) || item.getName(new ItemStack(item)).getString().toLowerCase().contains(lower)) {
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (editingButton != null && isEditorOpen) {
            ...
            if (mouseX >= editorLeft && mouseX <= editorLeft + editorWidth &&
                ...
                boolean inCmd = (mouseX >= editorLeft + 7 && mouseX <= editorLeft + 7 + commandTextField.width() &&
                        mouseY >= cmdY && mouseY <= cmdY + commandTextField.getHeight());
                boolean inIcon = (mouseX >= editorLeft + 7 && mouseX <= editorLeft + 7 + iconTextField.width() &&
                ...
                if (currentMode == FilterMode.SKULLS) {
                    ...
                    inSkull = (mouseX >= editorLeft + 7 && mouseX <= editorLeft + 7 + addSkullField.width() &&
            ...
            if (mouseX >= x && mouseX <= x + 18 && mouseY >= y && mouseY <= y + 18) {
                if (editingButton != btn) {
                    ...
                    if (!isEditorOpen) {
                        ...
                        commandTextField.setCursorPosition(btn.command.length());
